...
void OLED_init(void)
{
    ...
    oled_write_byte(0xAE, OLED_CMD);    //display off，关闭OLED显示
    oled_write_byte(0x20, OLED_CMD);    //Set Memory Addressing Mode，设置OLED的内存地址模式为页地址模式。	
    ...
    oled_write_byte(0xb0, OLED_CMD);    //Set Page Start Address for Page Addressing Mode,0-7，设置起点页地址为第一页地址
    oled_write_byte(0xc8, OLED_CMD);    //Set COM Output Scan Direction，设置SSD1306的COM扫描方式
    oled_write_byte(0x00, OLED_CMD);    //---set low column address，设置页地址的行起始地址，其中0x00设置行起始地址的低4位
    oled_write_byte(0x10, OLED_CMD);    //---set high column address，0x10设置行起始地址的高4位。
    oled_write_byte(0x40, OLED_CMD);    //--set start line address，设置起始线地址
    oled_write_byte(0x81, OLED_CMD);    //--set contrast control register，设置OLED亮度
    oled_write_byte(0xff, OLED_CMD);    //brightness 0x00~0xff
    oled_write_byte(0xa1, OLED_CMD);    //--set segment re-map 0 to 127，设置Segment重定向
    oled_write_byte(0xa6, OLED_CMD);    //--set normal display，设置正常显示，不反转显示
    oled_write_byte(0xa8, OLED_CMD);    //--set multiplex ratio(1 to 64)，设置多态比
    oled_write_byte(0x3F, OLED_CMD);    //设置63（0x3F）
    oled_write_byte(0xa4, OLED_CMD);    //0xa4,Output follows RAM content;0xa5,Output ignores RAM content，设置输出根据GDDRAM内容
    oled_write_byte(0xd3, OLED_CMD);    //-set display offset，设置显示偏移
    ...
    oled_write_byte(0xaf, OLED_CMD);    //--turn on oled panel，开启OLED显示
}
