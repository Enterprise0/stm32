...
#if OLED_SPI
#define	OLED_RES	1	//low to high, reset.或者直连高电平
#if OLED_RES
...
#define OLED_DC_HIGH	HAL_GPIO_WritePin(OLED_DC_GPIO_Port, OLED_DC_Pin, GPIO_PIN_SET); // OLED_CMD
#define OLED_DC_LOW		HAL_GPIO_WritePin(OLED_DC_GPIO_Port, OLED_DC_Pin, GPIO_PIN_RESET); // OLED_DATA
#endif
...
/**
  * @brief          显示一个字符串
  * @param[in]      row: 字符串的开始行 0~4
  * @param[in]      col: 字符串的开始列 0~20
  * @param[in]      chr: 字符串
  * @retval         none
  */
...
/**
  * @brief          格式输出
  * @param[in]      row: 开始行，0 <= row <= 4;
  * @param[in]      col: 开始列， 0 <= col <= 20;
